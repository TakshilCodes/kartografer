...
export default function NewTripClient() {
  ...
  const specialNotesCharacterCount = countNonWhitespaceCharacters(specialNotes);

  const [isGenerating, setIsGenerating] = useState(false);
  const [progress, setProgress] = useState(0);
  const [generationDayCount, setGenerationDayCount] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const steps =
      generationDayCount && generationDayCount > 7
        ? [
            {
              progress: 10,
              message: `Preparing your ${generationDayCount}-day trip...`,
            },
            {
              progress: 24,
              message: "Creating the route and daily structure...",
            },
            {
              progress: 42,
              message: "Building several itinerary sections in parallel...",
            },
            {
              progress: 60,
              message: "Adding stays, meals, activities, and options...",
            },
            {
              progress: 78,
              message: "Checking the complete itinerary and budget...",
            },
            {
              progress: 88,
              message: "Saving your long-trip plan...",
            },
          ]
        : [
      {
    ...
    const interval = window.setInterval(() => {
      ...
      if (!step) {
        if (generationDayCount && generationDayCount > 7) {
          setLoadingMessage(
            "Still building your long-trip plan. This can take a little longer.",
          );
        }
  ...
  }, [isGenerating, generationDayCount]);
  ...
  if (isGenerating) {
    return (
      <TripGenerationLoading
        ...
        isLongTrip={(generationDayCount ?? 0) > 7}
      />
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (mode === "ai") {
      ...
      const isLongTrip = Number.isFinite(requestedDays) && requestedDays > 7;

      setGenerationDayCount(isLongTrip ? requestedDays : null);
      setIsGenerating(true);
      setProgress(5);
      setLoadingMessage(
        isLongTrip
          ? `Building your ${requestedDays}-day itinerary in several sections...`
          : "Checking your trip details...",
      );
    }
            ...
            <div className="mt-4 mb-10 sm:mb-0">
              ...
              <div className="mt-2 flex flex-col gap-1.5 text-xs font-semibold text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                <span className="leading-5">
                  Keep requests short and relevant to this trip.
                </span>
              ...
              <div className="sm:rounded-2xl sm:bg-dashboard/60 sm:p-4 lg:flex lg:items-center lg:justify-between lg:gap-6">
                <div className="hidden min-w-0 sm:block">
                  ...
                  <p className="mt-1 hidden max-w-xl text-xs font-semibold leading-5 text-secondary-foreground sm:block">
                    Start with an empty itinerary and plan manually, or let AI
                    generate the first version for you.
                  </p>
                </div>
