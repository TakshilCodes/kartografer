...
const durationFilterOptions: Option[] = [
  ...
  ...durationOptions.map((option) => ({
    ...
    value: option.value,
    description:
      option.value === "15-plus" ? "Long-form itineraries" : "Shortlist by days",
  })),
];
...
export default function ExploreFilters({ query }: { query: ExploreQuery }) {
  ...
  return (
    <form
      ...
      className="relative overflow-visible rounded-[30px] border border-border bg-card/92 p-3 shadow-[0_18px_55px_rgba(81,49,23,0.08)] backdrop-blur sm:p-4"
    >
      <div className="mb-3 flex min-h-9 items-center justify-between gap-3 px-1">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-black text-foreground">Find an itinerary</p>
        </div>
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,0.9fr)_150px_150px_150px_165px_auto] lg:items-end">
        ...
        <ExploreDropdown name="duration" label="Duration" value={duration} onChange={setDuration} options={durationFilterOptions} />
        <ExploreDropdown name="budget" label="Budget" value={budget} onChange={setBudget} options={budgetFilterOptions} />
        <ExploreDropdown name="style" label="Style" value={style} onChange={setStyle} options={styleFilterOptions} />
        <ExploreDropdown name="sort" label="Sort" value={sort} onChange={setSort} options={sortFilterOptions} />
...
function ExploreDropdown({
  ...
  return (
    <div className="relative">
      ...
      {isOpen ? (
        <div className="absolute left-0 right-0 top-[calc(100%+0.55rem)] z-60 overflow-hidden rounded-[22px] border border-border bg-card p-1.5 shadow-[0_20px_55px_rgba(81,49,23,0.16)]">
              ...
              return (
                ...
                >
                  <span className="min-w-0">
                    ...
                    {option.description ? (
                      <span className={`mt-0.5 block line-clamp-1 text-xs font-semibold ${isSelected ? "text-primary-foreground/75" : "text-muted-foreground"}`}>
                        {option.description}
                      </span>
                    ) : null}
                  </span>
