...
export default function LandingProductStory() {
  ...
  return (
    <section id="product-story" className="relative overflow-hidden border-b border-[#e7d9c6] bg-white px-4 py-24 sm:px-6 lg:px-8 lg:py-32">
      <div className="landing-map-grid pointer-events-none absolute inset-0 opacity-20" />
        ...
        <div className="mt-14 grid gap-12 lg:grid-cols-[0.74fr_1.26fr] lg:items-center">
                    ...
                    {active ? (
                      <span className="absolute left-12 right-3 -top-px h-0.5 overflow-visible bg-[#eadbc5] sm:left-14">
                        <span
                          ...
                          className="landing-story-progress landing-story-progress-route relative block h-full bg-[#704522]"
                          style={{ animationDuration: `${STEP_DURATION}ms` }}
                        >
                          <span className="landing-story-progress-head absolute -right-1 -top-0.75 h-2 w-2 rounded-full bg-[#a97538] shadow-[0_0_0_4px_rgba(169,117,56,0.14)]" />
                        </span>
                      </span>
                    ) : null}
          ...
          >
            <div className="relative min-h-170 overflow-hidden rounded-[34px] bg-[#f8efe2] p-3 sm:p-5">
              <div className="landing-map-grid pointer-events-none absolute inset-0 opacity-40" />
              ...
              <div
                key={`${activeStep}-${cycleVersion}`}
                className={`landing-story-stage relative mt-3 ${
                  activeStep === 2
                    ? "min-h-260 md:min-h-150"
                    : "min-h-180 sm:min-h-147.5"
                }`}
              >
                {activeStep === 0 ? <TripFormDemo /> : null}
                {activeStep === 1 ? <ItineraryPreviewDemo /> : null}
                {activeStep === 2 ? <EditWorkspaceDemo /> : null}
              </div>
            </div>
          </BorderGlow>
        </div>
...
function TripFormDemo() {
  ...
  return (
    <div className="landing-story-panel absolute inset-0 overflow-hidden rounded-2xl border border-[#e2d0b7] bg-white shadow-[0_20px_55px_rgba(83,51,24,0.12)]">
      ...
      <div className="grid gap-3 p-4 sm:grid-cols-2 sm:p-5">
        {fields.map((field, index) => (
          <div key={field.label} className="landing-autofill-field rounded-2xl border border-[#dfcdb3] bg-[#fffaf3] p-3.5" style={{ animationDelay: `${450 + index * 760}ms` }}>
            ...
            <div className="mt-2 flex items-center gap-2 text-[#5b351a]">
              ...
              <span className="landing-autofill-value truncate text-sm font-black" style={{ animationDelay: `${650 + index * 760}ms` }}>{field.value}</span>
              <span className="landing-typing-caret" style={{ animationDelay: `${650 + index * 760}ms` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="px-4 pb-5 pt-4 sm:px-5">
        <button type="button" className="landing-generate-button flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#5b351a] text-sm font-black text-white shadow-[0_14px_30px_rgba(91,53,26,0.2)]">
          <Sparkles className="h-4 w-4" /> Generate trip
        </button>
      </div>

      <div className="landing-generation-overlay absolute inset-0 z-20 flex flex-col items-center justify-center bg-[#fffdf9]/96 px-5 text-center backdrop-blur-sm">
        <div className="relative h-28 w-full max-w-sm">
          <svg className="h-full w-full" viewBox="0 0 340 112" fill="none" aria-hidden="true">
            ...
            <path className="landing-generation-route" d="M25 80 C95 10 160 105 230 37 C265 4 290 35 315 20" stroke="#704522" strokeWidth="3" strokeDasharray="9 10" strokeLinecap="round" />
          </svg>
        ...
        <div className="mt-5 grid w-full max-w-md grid-cols-2 gap-2 text-left">
          {["Mapping route...", "Selecting stays...", "Planning meals...", "Estimating cost..."].map((status, index) => (
            <div key={status} className="landing-build-status flex items-center gap-2 rounded-xl border border-[#eadcca] bg-white px-3 py-2.5" style={{ animationDelay: `${4850 + index * 380}ms` }}>
...
function ItineraryPreviewDemo() {
  ...
  return (
    <div className="landing-story-panel absolute inset-0 overflow-hidden rounded-2xl border border-[#e2d0b7] bg-white shadow-[0_20px_55px_rgba(83,51,24,0.12)]">
      <div className="landing-map-grid absolute inset-0 opacity-30" />
      ...
      <div key={day.title} className="landing-preview-day relative grid gap-4 p-4 sm:p-5 md:grid-cols-[minmax(0,1fr)_155px]">
        <div className="min-w-0">
          ...
          <div className="mt-4 space-y-2.5">
            {day.items.map((item, index) => <PreviewItem key={item.title} {...item} delay={index * 90} />)}
          </div>
        </div>
...
function EditWorkspaceDemo() {
  return (
    <div className="landing-story-panel landing-edit-workspace absolute inset-0 overflow-hidden rounded-2xl border border-[#e2d0b7] bg-[#f7efe3] shadow-[0_20px_55px_rgba(83,51,24,0.12)]">
      ...
      <div className="landing-edit-grid grid min-h-135 gap-px bg-[#dfcdb4] md:grid-cols-[155px_minmax(245px,1fr)_175px]">
        <OptionsPanel />

        <main className="landing-editor-panel min-w-0 bg-white p-3.5">
          ...
          <div className="mt-3 space-y-2">
            ...
            <div className="relative rounded-xl border border-[#d7b982] bg-[#fffaf0] p-3 shadow-[0_8px_22px_rgba(83,51,24,0.08)]">
              ...
              <div className="landing-action-menu absolute right-2 top-10 z-20 w-32 overflow-hidden rounded-xl border border-[#dcc6aa] bg-white p-1.5 shadow-[0_16px_35px_rgba(63,39,19,0.2)]">
...
function OptionsPanel() {
  return (
    <aside className="landing-options-panel bg-[#fffaf3] p-3">
      <div className="flex items-center gap-2">
      ...
      <div className="landing-options-grid mt-3 space-y-2">
        <OptionCard title="Budget cafe" meta="Meal | INR 550" />
        <OptionCard title="Local bus" meta="Transport | INR 180" />
        <OptionCard title="Alpine stay" meta="Stay | INR 2,800" />
      </div>
    </aside>
  );
}
function ChatPanel() {
  return (
    <aside className="landing-chat-panel flex min-h-0 flex-col bg-[#fffdf9] p-3">
      ...
      <div className="flex-1 space-y-2 overflow-hidden py-3">
        ...
        <div className="landing-thinking-bubble w-fit rounded-2xl rounded-bl-md border border-[#e3d2ba] bg-white px-3 py-2 text-[9px] font-bold text-[#765f49]">Thinking<span>.</span><span>.</span><span>.</span></div>
      </div>
...
function PreviewItem({ type, title, icon: Icon, tone, delay }: { type: string; title: string; icon: typeof MapPin; tone: string; delay: number }) {
  ...
  return <div className="landing-preview-item flex items-center gap-3 rounded-xl border border-[#eadcca] bg-white p-3" style={{ animationDelay: `${delay}ms` }}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${tones[tone]}`}><Icon className="h-3.5 w-3.5" /></span><div className="min-w-0"><p className="text-[8px] font-black uppercase text-[#9a7855]">{type}</p><p className="truncate text-[11px] font-black text-[#2d1e11]">{title}</p></div></div>;
}
...
function OptionCard({ title, meta }: { title: string; meta: string }) {
  return <div className="rounded-xl border border-[#e0cdb2] bg-white p-2.5"><p className="text-[9px] font-black text-[#2d1e11]">{title}</p><p className="mt-1 text-[8px] font-semibold text-[#907455]">{meta}</p><button type="button" className="mt-2 w-full rounded-lg bg-[#f2e4d0] py-1.5 text-[8px] font-black text-[#65401f]">+ Add to plan</button></div>;
}